...
describe('Overview page', () => {
  ...
  it('Show correct errand table header and correct ammount of errands', () => {
    ...
    headerRow.get('th').eq(1).find('span').first().should('have.text', 'Rapportnummer');
    headerRow.get('th').eq(2).find('span').first().should('have.text', 'Typ av rapport');
    headerRow.get('th').eq(3).find('span').first().should('have.text', 'Rapporterat');
