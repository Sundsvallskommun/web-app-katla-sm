...
| Stable person search and compact people | Typing never inserts another button beside the field. A successful search replaces Search with a labelled reset action; editing a query clears its stale search error. Name and contact details share one column in `ListItem`, with a visible secondary remove button. The former card and its unused extra-content path are removed. | `StakeholderList`, `StakeholderRow` |
